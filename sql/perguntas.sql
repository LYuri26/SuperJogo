USE superjogo;

-- BLOCO 1 (1-25)
INSERT INTO
    perguntas (
        pergunta,
        resposta_correta,
        opcao1,
        opcao2,
        opcao3,
        opcao4,
        nivel_dificuldade
    )
VALUES
    (
        'Qual é a capital do Brasil?',
        'Brasília',
        'São Paulo',
        'Rio de Janeiro',
        'Brasília',
        'Salvador',
        'facil'
    ),
    (
        'Quantos segundos há em um minuto?',
        '60',
        '50',
        '60',
        '70',
        '100',
        'facil'
    ),
    (
        'Qual é a cor do céu em um dia claro?',
        'Azul',
        'Azul',
        'Verde',
        'Amarelo',
        'Vermelho',
        'facil'
    ),
    (
        'Quantos lados tem um quadrado?',
        '4',
        '3',
        '4',
        '5',
        '6',
        'facil'
    ),
    (
        'Qual animal é conhecido como "rei da selva"?',
        'Leão',
        'Tigre',
        'Elefante',
        'Leão',
        'Girafa',
        'facil'
    ),
    (
        'Qual é o planeta mais próximo do Sol?',
        'Mercúrio',
        'Vênus',
        'Terra',
        'Marte',
        'Mercúrio',
        'facil'
    ),
    (
        'Qual é a moeda oficial do Brasil?',
        'Real',
        'Dólar',
        'Real',
        'Euro',
        'Peso',
        'facil'
    ),
    (
        'Em que continente fica o Egito?',
        'África',
        'África',
        'Ásia',
        'Europa',
        'América',
        'facil'
    ),
    (
        'Qual é a primeira letra do alfabeto?',
        'A',
        'B',
        'C',
        'A',
        'D',
        'facil'
    ),
    (
        'Qual é o resultado de 2+2?',
        '4',
        '3',
        '4',
        '5',
        '6',
        'facil'
    ),
    (
        'Que cor resulta da mistura de azul e amarelo?',
        'Verde',
        'Roxo',
        'Laranja',
        'Verde',
        'Preto',
        'facil'
    ),
    (
        'Quantos dias tem uma semana?',
        '7',
        '5',
        '6',
        '7',
        '8',
        'facil'
    ),
    (
        'Qual fruta é conhecida por ser amarela e curvada?',
        'Banana',
        'Maçã',
        'Banana',
        'Uva',
        'Pera',
        'facil'
    ),
    (
        'Quem pintou a Mona Lisa?',
        'Leonardo da Vinci',
        'Leonardo da Vinci',
        'Michelangelo',
        'Van Gogh',
        'Picasso',
        'facil'
    ),
    (
        'Qual é o idioma oficial do Brasil?',
        'Português',
        'Inglês',
        'Português',
        'Espanhol',
        'Francês',
        'facil'
    ),
    (
        'Qual é o maior oceano do mundo?',
        'Oceano Pacífico',
        'Oceano Atlântico',
        'Oceano Índico',
        'Oceano Pacífico',
        'Oceano Ártico',
        'facil'
    ),
    (
        'Quantas horas há em um dia?',
        '24',
        '12',
        '24',
        '48',
        '60',
        'facil'
    ),
    (
        'Qual é a capital da Argentina?',
        'Buenos Aires',
        'Buenos Aires',
        'Rosário',
        'Mendoza',
        'Córdoba',
        'facil'
    ),
    (
        'Qual animal bota ovos?',
        'Galinha',
        'Gato',
        'Galinha',
        'Cachorro',
        'Cavalo',
        'facil'
    ),
    (
        'O que usamos para escrever no quadro negro?',
        'Giz',
        'Caneta',
        'Giz',
        'Lápis',
        'Pincel',
        'facil'
    ),
    (
        'Qual planeta é conhecido como planeta vermelho?',
        'Marte',
        'Marte',
        'Júpiter',
        'Saturno',
        'Netuno',
        'facil'
    ),
    (
        'Qual é o oposto de alto?',
        'Baixo',
        'Grande',
        'Pequeno',
        'Baixo',
        'Médio',
        'facil'
    ),
    (
        'Quantos meses tem um ano?',
        '12',
        '10',
        '11',
        '12',
        '13',
        'facil'
    ),
    (
        'O que usamos para medir o tempo?',
        'Relógio',
        'Calendário',
        'Relógio',
        'Régua',
        'Bússola',
        'facil'
    );

-- BLOCO 2 (26-50)
INSERT INTO
    perguntas (
        pergunta,
        resposta_correta,
        opcao1,
        opcao2,
        opcao3,
        opcao4,
        nivel_dificuldade
    )
VALUES
    (
        'Qual é o principal ingrediente do pão?',
        'Farinha',
        'Farinha',
        'Açúcar',
        'Sal',
        'Manteiga',
        'facil'
    ),
    (
        'Qual é o maior país do mundo?',
        'Rússia',
        'Rússia',
        'Canadá',
        'China',
        'Brasil',
        'facil'
    ),
    (
        'Qual a cor da bandeira do Japão?',
        'Branca com círculo vermelho',
        'Branca com círculo vermelho',
        'Branca com estrela azul',
        'Vermelha com sol',
        'Verde com círculo branco',
        'facil'
    ),
    (
        'Qual esporte é jogado com uma bola e cesta?',
        'Basquete',
        'Vôlei',
        'Basquete',
        'Futebol',
        'Handebol',
        'facil'
    ),
    (
        'Qual é o maior mamífero do mundo?',
        'Baleia-azul',
        'Baleia-azul',
        'Elefante',
        'Hipopótamo',
        'Orca',
        'facil'
    ),
    (
        'Qual é o símbolo químico da água?',
        'H2O',
        'H2O',
        'O2',
        'CO2',
        'NaCl',
        'facil'
    ),
    (
        'Qual país inventou o samba?',
        'Brasil',
        'Brasil',
        'Portugal',
        'Cuba',
        'México',
        'facil'
    ),
    (
        'Qual instrumento mede a temperatura?',
        'Termômetro',
        'Régua',
        'Bússola',
        'Termômetro',
        'Barômetro',
        'facil'
    ),
    (
        'Qual é o continente do Brasil?',
        'América do Sul',
        'América do Norte',
        'América do Sul',
        'Europa',
        'Ásia',
        'facil'
    ),
    (
        'Quantas cores tem o arco-íris?',
        '7',
        '6',
        '7',
        '8',
        '5',
        'facil'
    ),
    (
        'Qual é o animal símbolo da Austrália?',
        'Canguru',
        'Canguru',
        'Coala',
        'Pinguim',
        'Cavalo',
        'facil'
    ),
    (
        'O que é maior: o Sol ou a Terra?',
        'Sol',
        'Terra',
        'Lua',
        'Sol',
        'Júpiter',
        'facil'
    ),
    (
        'Qual é a capital de Portugal?',
        'Lisboa',
        'Lisboa',
        'Porto',
        'Coimbra',
        'Faro',
        'facil'
    ),
    (
        'Qual é o formato da bola de futebol?',
        'Esférico',
        'Esférico',
        'Cúbico',
        'Oval',
        'Triangular',
        'facil'
    ),
    (
        'Qual é a cor do leite?',
        'Branco',
        'Branco',
        'Preto',
        'Amarelo',
        'Verde',
        'facil'
    ),
    (
        'Qual é a língua oficial do Japão?',
        'Japonês',
        'Japonês',
        'Chinês',
        'Coreano',
        'Inglês',
        'facil'
    ),
    (
        'Quem foi o primeiro presidente do Brasil?',
        'Deodoro da Fonseca',
        'Deodoro da Fonseca',
        'Getúlio Vargas',
        'Dom Pedro II',
        'Marechal Floriano',
        'medio'
    ),
    (
        'Qual é a fórmula química do sal de cozinha?',
        'NaCl',
        'NaCl',
        'KCl',
        'NaOH',
        'HCl',
        'medio'
    ),
    (
        'Quem escreveu "Dom Casmurro"?',
        'Machado de Assis',
        'Machado de Assis',
        'José de Alencar',
        'Clarice Lispector',
        'Graciliano Ramos',
        'medio'
    ),
    (
        'Qual país venceu a Copa do Mundo de 2002?',
        'Brasil',
        'Brasil',
        'Alemanha',
        'França',
        'Itália',
        'medio'
    ),
    (
        'Qual é a capital da Austrália?',
        'Camberra',
        'Sydney',
        'Camberra',
        'Melbourne',
        'Perth',
        'medio'
    ),
    (
        'Qual órgão do corpo humano filtra o sangue?',
        'Rim',
        'Pulmão',
        'Rim',
        'Fígado',
        'Coração',
        'medio'
    ),
    (
        'Em que ano o homem pisou na Lua pela primeira vez?',
        '1969',
        '1968',
        '1969',
        '1970',
        '1972',
        'medio'
    ),
    (
        'Qual pintor é famoso por cortar parte da própria orelha?',
        'Van Gogh',
        'Van Gogh',
        'Picasso',
        'Monet',
        'Dalí',
        'medio'
    );

-- BLOCO 3 (51-75)
INSERT INTO
    perguntas (
        pergunta,
        resposta_correta,
        opcao1,
        opcao2,
        opcao3,
        opcao4,
        nivel_dificuldade
    )
VALUES
    (
        'Quem foi Albert Einstein?',
        'Físico teórico',
        'Pintor',
        'Físico teórico',
        'Músico',
        'Poeta',
        'medio'
    ),
    (
        'Quantos ossos tem o corpo humano adulto?',
        '206',
        '210',
        '206',
        '200',
        '180',
        'medio'
    ),
    (
        'Qual é o maior deserto do mundo?',
        'Deserto da Antártida',
        'Saara',
        'Deserto da Antártida',
        'Kalahari',
        'Gobi',
        'medio'
    ),
    (
        'Quem pintou "A Última Ceia"?',
        'Leonardo da Vinci',
        'Michelangelo',
        'Leonardo da Vinci',
        'Raphael',
        'Caravaggio',
        'medio'
    ),
    (
        'Qual é o metal usado para fabricar moedas de 1 real?',
        'Aço inoxidável com revestimento de bronze',
        'Alumínio',
        'Cobre',
        'Aço inoxidável com revestimento de bronze',
        'Latão',
        'medio'
    ),
    (
        'Quem é o autor de "O Pequeno Príncipe"?',
        'Antoine de Saint-Exupéry',
        'Antoine de Saint-Exupéry',
        'Victor Hugo',
        'Jules Verne',
        'Charles Dickens',
        'medio'
    ),
    (
        'Qual é o rio mais longo do mundo?',
        'Rio Nilo',
        'Rio Nilo',
        'Rio Amazonas',
        'Rio Yangtzé',
        'Rio Mississipi',
        'medio'
    ),
    (
        'Quem descobriu o Brasil?',
        'Pedro Álvares Cabral',
        'Pedro Álvares Cabral',
        'Cristóvão Colombo',
        'Vasco da Gama',
        'Fernão de Magalhães',
        'medio'
    ),
    (
        'O que mede a escala Richter?',
        'Intensidade de terremotos',
        'Velocidade do vento',
        'Intensidade de terremotos',
        'Temperatura',
        'Profundidade do mar',
        'medio'
    ),
    (
        'Qual o idioma oficial da China?',
        'Mandarim',
        'Mandarim',
        'Cantonês',
        'Inglês',
        'Japonês',
        'medio'
    ),
    (
        'Qual é o maior planeta do sistema solar?',
        'Júpiter',
        'Júpiter',
        'Saturno',
        'Urano',
        'Netuno',
        'medio'
    ),
    (
        'Quem inventou a lâmpada elétrica?',
        'Thomas Edison',
        'Thomas Edison',
        'Nikola Tesla',
        'Alexander Graham Bell',
        'James Watt',
        'medio'
    ),
    (
        'Qual é a capital do Canadá?',
        'Ottawa',
        'Toronto',
        'Vancouver',
        'Ottawa',
        'Montreal',
        'medio'
    ),
    (
        'Quem foi o primeiro homem a viajar para o espaço?',
        'Yuri Gagarin',
        'Neil Armstrong',
        'Buzz Aldrin',
        'Yuri Gagarin',
        'John Glenn',
        'medio'
    ),
    (
        'Qual é a montanha mais alta do mundo?',
        'Monte Everest',
        'Monte Everest',
        'K2',
        'Kangchenjunga',
        'Makalu',
        'medio'
    ),
    (
        'Quem compôs o Hino Nacional Brasileiro?',
        'Francisco Manuel da Silva',
        'Francisco Manuel da Silva',
        'Carlos Gomes',
        'Heitor Villa-Lobos',
        'Chiquinha Gonzaga',
        'medio'
    ),
    (
        'O que é fotossíntese?',
        'Processo de produção de alimento pelas plantas',
        'Respiração das plantas',
        'Processo de produção de alimento pelas plantas',
        'Crescimento das raízes',
        'Formação das flores',
        'medio'
    ),
    (
        'Qual cientista formulou as leis do movimento?',
        'Isaac Newton',
        'Isaac Newton',
        'Galileu Galilei',
        'Albert Einstein',
        'Johannes Kepler',
        'medio'
    ),
    (
        'Qual país tem a maior população do mundo?',
        'China',
        'China',
        'Índia',
        'EUA',
        'Indonésia',
        'medio'
    ),
    (
        'Quem foi Napoleão Bonaparte?',
        'Imperador francês',
        'Imperador francês',
        'Rei da Inglaterra',
        'General romano',
        'Pintor espanhol',
        'medio'
    ),
    (
        'Qual é o elemento químico representado por Fe?',
        'Ferro',
        'Ferro',
        'Flúor',
        'Frâncio',
        'Fermio',
        'medio'
    ),
    (
        'Qual cidade é conhecida como a "Cidade Eterna"?',
        'Roma',
        'Roma',
        'Atenas',
        'Paris',
        'Jerusalém',
        'medio'
    ),
    (
        'Em que país fica a Torre Eiffel?',
        'França',
        'França',
        'Itália',
        'Espanha',
        'Alemanha',
        'medio'
    ),
    (
        'Quem escreveu "Os Lusíadas"?',
        'Luís de Camões',
        'Luís de Camões',
        'Fernando Pessoa',
        'José Saramago',
        'Eça de Queirós',
        'medio'
    );

-- BLOCO 4 (76-100)
INSERT INTO
    perguntas (
        pergunta,
        resposta_correta,
        opcao1,
        opcao2,
        opcao3,
        opcao4,
        nivel_dificuldade
    )
VALUES
    (
        'Qual é o continente mais frio?',
        'Antártida',
        'Ásia',
        'Antártida',
        'Europa',
        'América',
        'medio'
    ),
    (
        'Qual é a capital da Rússia?',
        'Moscou',
        'São Petersburgo',
        'Moscou',
        'Kiev',
        'Novosibirsk',
        'medio'
    ),
    (
        'O que é um haicai?',
        'Poema curto de origem japonesa',
        'Poema longo',
        'Poema curto de origem japonesa',
        'Música tradicional',
        'Dança típica',
        'medio'
    ),
    (
        'Qual o nome do satélite natural da Terra?',
        'Lua',
        'Lua',
        'Sol',
        'Estrela Polar',
        'Cometa Halley',
        'medio'
    ),
    (
        'Qual é a capital da Espanha?',
        'Madri',
        'Madri',
        'Barcelona',
        'Sevilha',
        'Valência',
        'medio'
    ),
    (
        'Qual é a fórmula da Teoria da Relatividade?',
        'E=mc²',
        'E=mc²',
        'F=ma',
        'V=d/t',
        'p=mv',
        'difícil'
    ),
    (
        'Quem desenvolveu a teoria da evolução?',
        'Charles Darwin',
        'Charles Darwin',
        'Gregor Mendel',
        'Jean-Baptiste Lamarck',
        'Louis Pasteur',
        'difícil'
    ),
    (
        'Qual é o elemento químico mais leve?',
        'Hidrogênio',
        'Hidrogênio',
        'Hélio',
        'Lítio',
        'Oxigênio',
        'difícil'
    ),
    (
        'Quem foi o autor de "A República"?',
        'Platão',
        'Platão',
        'Aristóteles',
        'Sócrates',
        'Heráclito',
        'difícil'
    ),
    (
        'Qual é a capital da Mongólia?',
        'Ulan Bator',
        'Ulan Bator',
        'Astana',
        'Pequim',
        'Hanoi',
        'difícil'
    ),
    (
        'Qual físico propôs o princípio da incerteza?',
        'Werner Heisenberg',
        'Werner Heisenberg',
        'Niels Bohr',
        'Paul Dirac',
        'Max Planck',
        'difícil'
    ),
    (
        'Qual filósofo é conhecido por dizer "Penso, logo existo"?',
        'René Descartes',
        'René Descartes',
        'Immanuel Kant',
        'John Locke',
        'Friedrich Nietzsche',
        'difícil'
    ),
    (
        'Qual país é o maior produtor de cacau do mundo?',
        'Costa do Marfim',
        'Costa do Marfim',
        'Brasil',
        'Gana',
        'Indonésia',
        'difícil'
    ),
    (
        'Quem compôs a ópera "O Barbeiro de Sevilha"?',
        'Gioachino Rossini',
        'Gioachino Rossini',
        'Giuseppe Verdi',
        'Wolfgang Amadeus Mozart',
        'Gaetano Donizetti',
        'difícil'
    ),
    (
        'Em que ano começou a Primeira Guerra Mundial?',
        '1914',
        '1914',
        '1918',
        '1939',
        '1920',
        'difícil'
    ),
    (
        'Qual é a capital do Butão?',
        'Thimphu',
        'Thimphu',
        'Catmandu',
        'Daca',
        'Colombo',
        'difícil'
    ),
    (
        'Quem formulou as leis da hereditariedade?',
        'Gregor Mendel',
        'Gregor Mendel',
        'Charles Darwin',
        'Louis Pasteur',
        'Francis Crick',
        'difícil'
    ),
    (
        'Qual é a língua oficial do Irã?',
        'Persa',
        'Persa',
        'Árabe',
        'Curdo',
        'Turco',
        'difícil'
    ),
    (
        'Quem escreveu "A Divina Comédia"?',
        'Dante Alighieri',
        'Dante Alighieri',
        'Boccaccio',
        'Petrarca',
        'Maquiavel',
        'difícil'
    ),
    (
        'Qual foi a primeira civilização a desenvolver a escrita?',
        'Sumérios',
        'Sumérios',
        'Egípcios',
        'Fenícios',
        'Maias',
        'difícil'
    ),
    (
        'Qual país tem a cidade de Timbuktu?',
        'Mali',
        'Mali',
        'Níger',
        'Sudão',
        'Chade',
        'difícil'
    ),
    (
        'Qual é o metal mais precioso do mundo?',
        'Ródio',
        'Ouro',
        'Ródio',
        'Platina',
        'Paládio',
        'difícil'
    ),
    (
        'Quem foi o líder soviético durante a Segunda Guerra Mundial?',
        'Josef Stalin',
        'Josef Stalin',
        'Vladimir Lenin',
        'Nikita Khrushchov',
        'Mikhail Gorbachev',
        'difícil'
    ),
    (
        'Qual é a distância média da Terra ao Sol?',
        '150 milhões de km',
        '100 milhões de km',
        '200 milhões de km',
        '150 milhões de km',
        '300 milhões de km',
        'difícil'
    ),
    (
        'Qual foi a primeira mulher a ganhar um Prêmio Nobel?',
        'Marie Curie',
        'Marie Curie',
        'Rosalind Franklin',
        'Ada Lovelace',
        'Lise Meitner',
        'difícil'
    );